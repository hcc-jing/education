...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <script type="text/javascript" src="__THEME__/js/user/user_common.js"></script>
  ...
  padding: 0 13px 0px 40px;background: url(&#39;..__THEME__/style/images/mobile.png&#39;) 12px 7px no-repeat #e03f27; display:none;">手机看直播</a>
    ...
    <?php if(in_array($user['uid'],$user)): ?><div class="top_right fr header_right"> <span class="guestname"><a href="javascript:void(0)" class="guest_ico">
        ...
        <img style="margin-top:5px;" src="image/Connect_logo_6.png" width="50" height="16" alt=""></a></span> -->
                        <span id="log_span"><a class="log on_log" href="admin_user/index.php" target="_blank">个人中心</a></span> <span id="reg_span"><a class="reg on_reg" href="javascript:;" onclick="logout()">退出</a></span>
        ...
        <div class="main_middle1 ">
            ...
            <div class="tool_bar ">
                <ul>
                    ...
                    <li>
                        <a href="/#redbag_choice" class=" showdiv bar_redbag">抢红包<span>￥<?php echo ($userLearnc['balance']); ?></span></a>
                    </li>
                    <?php if(($user[user_group_id] == 1) OR ($user[user_group_id] == 2)): ?><li><a href="javascript:void(0)"  class="bar_quanping" id="gundong">走马灯</a></li>
                        <script type="text/javascript">
                            <?php echo ($allscreen); ?>
                        </script><?php endif; ?>
                ...
                <div class="xialatiao ">
                <?php if(($user['user_group_id'] < 3) OR ($user['user_group_id'] == 3)): ?><select id="role_select">
                            <option value="<?php echo ($user['uname']); ?>" aid="<?php echo ($user['user_group_id']); ?>" uid="<?php echo ($user['uid']); ?>">我</option>
                        </select><?php endif; ?>
...
function init_screen(){
    ...
    $(".quanping").find("div").show().each(function(){
      var _width=$(this).text().length*57;
      var _left=$(window).width();
      var time=parseInt(18+_width*18/_left)*1000;
    ...
    /*  if(_width<_left){
        ...
        var time=26000;
        //$(this).css({width:_left});
      }*/
      
      $(this).css({width:_width});
      var _height=$(window).height();
      
      _top=parseInt(Math.random()*$(window).height());
      if(_top>=_height-100){
        _top=0;
      }
      
      if($(this).offset().left=="-"+_width){
        $(this).remove();
      }

      if($(this).offset().left==5){
        $(this).css({left:_left,top:_top});
        $(this).animate({left:"-"+_width+"px"},time,'linear',function(){
        });
      }
    });
}
