$(function () {
    ...
    if (ADMINID == 14) {
       /* var itime = $('.set_haibao').val();
        if(itime == ""){
            return false;
        }else{
            setInterval('haibao()', 60 * itime * 1000);
        }*/
        //alert(itime);
        setInterval('haibao()', 60 * 10 * 1000);
        if (VIEWSTATUS != '0') {
    ...
    $.ajax({
               ...
               success: function (data) {//如果调用php成功
                   if (data.content) {
                       ...
                       window.location.reload();
                       //notice('对不起，因为您多次违规操作使用融汇财经直播室，管理员已对您进行请出直播室处理！');
                   } else if (data == 'shenhe') {
                       notice('消息审核中。。。');
                   } else if (!isNaN(data)) {
                       WAIT = data;
